...
contract Dreidel {
    ...
    function computeGame() public view returns(uint8[]) {
        ...
        uint8[] storage pieceCount;
        uint8 pot = 0;
        for (uint i = 0; i < playerCount; i++)  pieceCount.push(PIECE_AMT);
        bytes32 game = generateRandomNumber();
        uint8 playerIndex = 0;
        uint8 bytesIndex = 0;
        bytes1 three = bytes1(3*16);
        while (pieceCount.length > 1 && bytesIndex < 32) {
            bytes1 curr = game[bytesIndex];
            for(uint8 j = 0; j < 4; j++) {
                bytes1 ourTurn = (curr & three);
                curr = curr << 2;
                if (ourTurn == 0) { //nun
                    playerIndex = (playerIndex + 1) % uint8(pieceCount.length); 
                    ...
                    pot+= remove;
                    if (pieceCount[playerIndex] == 0) delete pieceCount[playerIndex];
                }
            }
            bytesIndex++;
        }
